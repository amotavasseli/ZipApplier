...
namespace ZipApplier.ConsoleApp
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            //ZipRecruiter lists 20 job postings on each page. Below will determine how many pages need to be checked.
            int pages = 1;
            int extraPage = 0; 
            if(total > 20)
            ...
            var listings = doc.QuerySelectorAll("article.t_job_result");
            for (int i = 0; i < listings.Length; i++)
            {
                ...
                if (title.Contains("Senior")
                    ...
                    && title.Contains("Software Developer") //this needs to be changed with each search
                    )
                {
                    Job job = new Job();
                    job.Title = title;
